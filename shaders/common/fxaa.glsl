#ifndef C3D_FXAA_GLSL
#define C3D_FXAA_GLSL
...
vec4 fetch(vec2 uv) {
    return sample_texture_2d(fxaa_root.input_texture, fxaa_root.sampler_index, uv);
}
...
vec4 fxaa_color(FxaaRoot root, vec2 uv) {
    fxaa_root = root;
    vec2 inverse_size = 1.0 / vec2(root.width, root.height);
    return fxaa(uv, inverse_size);
}
