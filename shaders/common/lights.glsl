...
vec3 evaluate_standard_light(
    ...
    if (light.kind == LIGHT_DIRECTIONAL) {
        light_direction = -light.direction_cos_outer.xyz;
    } else {
        vec3 offset = light.position_range.xyz - position;
        float distance_squared = dot(offset, offset);
        if (distance_squared == 0.0) return vec3(0.0);

        light_direction = offset * inversesqrt(distance_squared);
        ...
        if (light.kind == LIGHT_SPOT) {
            attenuation *= spot_attenuation(
                dot(light.direction_cos_outer.xyz, -light_direction),
                light.cos_inner,
    ...
    vec3 response = evaluate_standard_brdf(surface, light_direction);
    return response * light.color_intensity.rgb * light.color_intensity.w * attenuation;
}
