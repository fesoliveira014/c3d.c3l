...
a filesystem; it is committed and checked like the registry table.
  ...
  scripts/build_shaders.py            compile every entry and write the registry and include tables
  scripts/build_shaders.py --check    compile every entry; fail if a committed table is out of date
"""
...
def load_manifest() -> tuple[list[str], list[Entry]]:
    ...
    for raw in manifest.get("entries", []):
        ...
        if not source.exists():
            raise ManifestError(f"{shader}: source {source.relative_to(ROOT).as_posix()} does not exist")
...
def main() -> int:
    ...
    log(f"compiled {len(entries)} stage(s), {len(includes)} include(s), tables {'checked' if arguments.check else 'written'}")
    return 0
