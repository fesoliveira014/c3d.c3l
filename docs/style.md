...
| `module c3d::<area>;` | One directory per architecture module: `maths`, `ecs`, `asset`, `scene`, `geometry`, `camera`, `material`, `light`, `anim`, `model`, `physics`, `platform`, `render`, `shader`, `rt`, `gui`. Public declarations in `<area>.c3i` where an interface file helps; otherwise one `.c3` per topic. |
| `module c3d::<area>::<sub>;` | Submodules named in the architecture: `asset::gltf`, `asset::fbx`, `asset::image`, `anim::ik`, `anim::retarget`, `render::post`, `gui::backend`, `ecs::store`. A submodule that records with the renderer's private helpers opens them with `import c3d::render @public;`. |
